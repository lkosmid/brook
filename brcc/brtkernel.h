...
class BRTGLSLKernelCode : public BRTGPUKernelCode
{
  public:
   BRTGLSLKernelCode(const FunctionDef& fDef) : BRTGPUKernelCode(fDef) {}
   ~BRTGLSLKernelCode() { /* Nothing, ~BRTKernelCode() does all the work */ }
   
   BRTKernelCode *dup0() const { return new BRTGLSLKernelCode(*this->fDef); }
   void printCode(std::ostream& out) const;
};

class BRTGLESKernelCode : public BRTGPUKernelCode
{
  public:
   BRTGLESKernelCode(const FunctionDef& fDef) : BRTGPUKernelCode(fDef) {}
   ~BRTGLESKernelCode() { /* Nothing, ~BRTKernelCode() does all the work */ }
   
   BRTKernelCode *dup0() const { return new BRTGLESKernelCode(*this->fDef); }
   void printCode(std::ostream& out) const;
};

class BRTCTMKernelCode : public BRTGPUKernelCode
{
  public:
   BRTCTMKernelCode(const FunctionDef& fDef) : BRTGPUKernelCode(fDef) {}
   ~BRTCTMKernelCode() { /* Nothing, ~BRTKernelCode() does all the work */ }
   
   BRTKernelCode *dup0() const { return new BRTCTMKernelCode(*this->fDef); }
   void printCode(std::ostream& out) const;
};
